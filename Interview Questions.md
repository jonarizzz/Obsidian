...
- Предположим, у нас есть задача вызвать некий внешний [[API (Application Programming Interface)||API]], например, для проверки личности клиента. Нам нужно построить интеграцию со сторонними организациями с нуля. Каждый вызов [[API (Application Programming Interface)||API]] стоит нам денег. Какие самые важные вещи вы бы сделали, чтобы убедиться, что все работает правильно? Что следует рассмотреть в первую очередь?
	- Технические аспекты (надежность и корректность)
		- **[[Логирование (Logging)||Логирование]] и [[Мониторинг (Monitoring)||мониторинг]]** – фиксировать [[HTTP Запрос (HTTP Request)||запросы]], ошибки, тайминги, настроить алерты.
		- **[[Идемпотентность (Idempotency)||Идемпотентность]] и повторные вызовы** – использовать [[Идемпотентность (Idempotency)||идемпотентные]] ключи, retry с экспоненциальной задержкой.
		- **[[Кэш (Cache)||Кэширование]]** – избегать повторных запросов, если данные уже проверены.
		- **Тайм-ауты и ограничение времени выполнения** – предотвращать зависания [[API (Application Programming Interface)||API]].
		- **Обработка ошибок и валидация** – учитывать нестандартные ошибки, проверять входные/выходные данные.
	- Архитектурные аспекты ([[Масштабирование (Скейлинг, Scaling)||масштабируемость]] и гибкость)
		- **[[Очередь (Queue)||Очереди]] и асинхронность** – вынести [[HTTP Запрос (HTTP Request)||вызовы]] в [[Очередь (Queue)||очередь]], если реальное время не критично.
		- **Rate Limiting** – ограничить количество [[HTTP Запрос (HTTP Request)||запросов]], использовать batch-вызовы.
		- **Выбор API-провайдера** – оценить SLA, поддержку, документацию.
	- Бизнес-аспекты (оптимизация затрат и рисков)
		- **Симуляция API перед запуском** – тестировать на моках и в sandbox.
		- **Контроль затрат** – минимизировать [[HTTP Запрос (HTTP Request)||вызовы]], оценить влияние на бизнес.
		- **Fallback-стратегия** – предусмотреть поведение при сбоях [[API (Application Programming Interface)||API]].
	- **Главное:** минимизировать избыточные [[HTTP Запрос (HTTP Request)||вызовы]], обеспечить стабильность работы и контролировать расходы.

- Как бы вы спроектировали функцию депозита/снятия средств со счёта в микросервисной архитектуре (на высоком уровне), где у вас есть платежная и основная службы, работающие независимо друг от друга с собственными базами данных. Какие механизмы блокировки использовать и как координировать межсервисную связь?
	- Использовать [[Сага (SAGA)||SAGA]] для согласованности
		- [[Оркестрация (Orchestration)||Оркестрация]] (лучше при сложных сценариях) → Account Service управляет процессом.
		- [[Хореография (Choreography)||Хореография]] (лучше для простых сценариев) → Payment Service и Account Service общаются через события ([[Кафка (Apache Kafka)||Kafka]]/[[{TODO} RabbitMQ||RabbitMQ]]).
	- Минимизировать [[Блокировки (Locking)||блокировки]], используя [[Оптимистичная Блокировка (Optimistic Locking)||оптимистичную блокировку]]
		- Хранить `balance_version` и проверять при обновлении (`UPDATE ... WHERE balance_version = X`).
		- Использовать [[Идемпотентность (Idempotency)||идемпотентные]] [[Транзакция (Transaction)||транзакции]], чтобы избежать дубликатов.
	- Обрабатывать сбои через `retry` и [[Очередь (Queue)||очереди]]
		- Повторять запросы с экспоненциальной задержкой при временных ошибках.
		- Если платеж не подтвержден, отправлять в [[Dead Letter Queue (DLQ)||Dead Letter Queue (DLQ)]].
	- Избегать гонок данных с помощью резервирования средств
		- Сначала замораживать средства в Account Service.
		- После успешного платежа — подтверждать списание.
		- Если платеж не удался — размораживать.
	- Использовать [[Распределённая Блокировка (Distributed Lock)||distributed lock]] (если нужно строгое выполнение операций)
		- [[{TODO} Redlock||Redis (Redlock)]] или [[PostgreSQL Advisory Lock||Postgres advisory locks]] для защиты от двойного выполнения.

---
## Memory Management

- [[Java Memory Model (JMM)||Java Memory Model]] = часть семантики [[{TODO} Java||Java]]. Описывает поведение [[Поток (Thread)||потоков]] в среде исполнения [[{TODO} Java||Java]].
- [[Stop-The-World (STW)||Stop-The-World]] = остановка всех [[Поток (Thread)||потоков]] [[{TODO} Java||Java]], чтобы [[Сборщик Мусора (Garbage Collector, GC)||GC]] удалил неиспользуемые [[Объект (Object)||объекты]].
	- Присутствует в:
		- [[Serial GC||Serial GC]]
...
- Когда [[Объект (Object)||объект]] становится подлежащим удалению [[Сборщик Мусора (Garbage Collector, GC)||Garbage Collector]]? Опишите как [[Сборщик Мусора (Garbage Collector, GC)||Garbage Collector]] находит подходящие [[Объект (Object)||объекты]].
	- [[Объект (Object)||Объект]] подлежит удалению когда он становится недостижим из корневого [[Объект (Object)||объекта]]. 
	- Как [[Сборщик Мусора (Garbage Collector, GC)||Сборщик Мусора]] находит подходящие [[Объект (Object)||объекты]]:
		- **Mark-and-Sweep:** Алгоритм помечает все достижимые [[Объект (Object)||объекты]], а затем очищает недостижимые.
		- **Reference Counting:** Учитывается количество [[Ссылочный Тип Данных (Сильная Ссылка, Strong Reference, Link)||ссылок]] на [[Объект (Object)||объект]]. Если счетчик равен нулю, [[Объект (Object)||объект]] подлежит удалению.
		- **Tracing Garbage Collection:** Использует алгоритм обхода [[{TODO} Граф (Graph)||графа]] для поиска достижимых [[Объект (Object)||объектов]].
- Что хранится в [[Heap (Область Памяти)||Heap-памяти]], а что – в [[Stack (Область Памяти)||Stack]]?
	- **[[Heap (Область Памяти)||Heap]]** хранит [[Объект (Object)||объекты]] и данные с динамическим временем жизни, доступные через [[Ссылочный Тип Данных (Сильная Ссылка, Strong Reference, Link)||ссылки]].
	- **[[Stack (Область Памяти)||Stack]]** хранит локальные переменные, [[Ссылочный Тип Данных (Сильная Ссылка, Strong Reference, Link)||ссылки на объекты]] и информацию о вызовах функций, с автоматическим управлением памятью (память освобождается при выходе из функции).
- Какие особые гарантии [[Java Memory Model (JMM)||JMM]] предоставляет для [[final||final]] полей [[Класс (Class)||класса]]?
	- **Инициализация**: Значение [[final||final]] поля инициализируется только один раз, гарантируя, что оно будет установлено до завершения конструктора.
	- **Видимость**: После инициализации [[final||final]] поля его значение будет видимо для всех [[Поток (Thread)||потоков]], которые читают это поле после завершения конструктора [[Объект (Object)||объекта]].
	- **Неизменность**: После инициализации значение [[final||final]] поля не может быть изменено, что предотвращает его перезапись в других [[Поток (Thread)||потоках]].
