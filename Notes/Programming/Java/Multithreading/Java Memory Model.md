...
- Хранилище данных:
	...
	- Локальные [[Кэш (Cache)||кэши]] (thread-local cache) — каждый поток может загружать копии переменных в локальные кэши (например, в регистры процессора).
- Взаимодействие потоков с памятью:
	- [[Поток (Thread)||Потоки]] читают и пишут переменные через [[Кэш (Cache)||кэши]] и основную память.
